...
def padding(x):
    if len(x)==1000:
        return x
    padded = np.tile(x[-1], (1000-len(x),1))
...
def generate_dataset(PADDING_DATASET=True,NORMALIZATION_OUTPUT=True,DRAW_DISTRIBUTION=False):
    ...
    for trajetory in train_list:
        ...
        if PADDING_DATASET:
            train_data.append(padding(data.values[:-1,0:50]))
        else:
    ...
    for trajetory in test_list:
        data = pd.read_table(trajetory, sep=';', header=None)
        if PADDING_DATASET:
            test_data.append(padding(data.values[:-1,0:50]))
        else:
    ...
    for trajetory in evaluate_list:
        data = pd.read_table(trajetory, sep=';', header=None)
        if PADDING_DATASET:
            evaluate_data.append(padding(data.values[:-1,0:50]))
        else:
...
if __name__ == '__main__':
    ...
    # dummy_input = torch.rand(15,101,8).long().to(device)
