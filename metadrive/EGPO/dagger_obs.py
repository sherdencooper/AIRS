...
def obs_process(obs,obs_file):
    to_write = ''
    for i in range(259):
        to_write+=str(obs[i])+';'
    obs_file.write(to_write+'\n')

def write_obs(env, model, evaluation_episode_num=30, device="cuda"):
    with torch.no_grad():
        print("... evaluation")
        episode_reward = 0
        episode_cost = 0
        ...
        obs_file = open('./observation/train/'+str(episode_num)+'.txt', 'w')
        while episode_num < evaluation_episode_num:
            ...
            obs_process(next_state,obs_file)
            state = next_state
            episode_reward += r
            reward += r
            ...
            if done:
                ...
                set_seed(episode_num)
                obs_file.write((str(reward)+';')*259)
                reward = 0
                obs_file.close()
                if episode_num < evaluation_episode_num:
                    obs_file = open('./observation/train/'+str(episode_num)+'.txt', 'w')
        res = dict(
...
if __name__ == "__main__":
    ...
    eval_res = write_obs(eval_env, agent, evaluation_episode_num=evaluation_episode_num)
    print(eval_res)
