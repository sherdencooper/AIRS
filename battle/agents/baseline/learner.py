...
def epsilon_greedy_search(
    ...
    render_file_index = 1
    if obs_path==None or seed==None:
        obs_save_path = '/home/jvy5516/project/battle/obs_file.txt'
    else:
        obs_save_path = obs_path + str(seed) + '.txt' 
    obs_file = open(obs_save_path,'w')
    for i_episode in range(1, episode_count + 1):
        ...
        if write_log:
            write_obs_process(obs_file,observation,init=True)
        total_reward = 0.0
        all_rewards = []
        all_availability = []
        learner.new_episode()
        ...
        for t in bar(range(1, 1 + iteration_count)):
            ...
            x = np.random.rand()
            if fidelity_test and t==replay_index:
                x = 0.0
            if x <= epsilon:
            ...
            if write_log:
                write_obs_process(obs_file,observation,gym_action, reward, action_type)
        ...
        if plot_episodes_length and draw_plot:
    ...
    wrapped_env.close()
    if write_log:
        reward_write = (str(total_reward) + ';') * 7 + str(total_reward)
        obs_file.write(reward_write)
    obs_file.close()
...
# def write_obs_process(obs_file,observation, gym_action=None, reward=None, action_type=None, init=False):
#     to_write = observation.copy()
#     to_write.pop('action_mask')
#     to_write.pop('explored_network')
#     to_write.pop('discovered_nodes_properties')
#     # to_write.pop('lateral_move')
#     to_write.pop('customer_data_found')
#     to_write.pop('escalation')
#     to_write.pop('probe_result')
#     to_write.pop('credential_cache_matrix')
#     to_write.pop('nodes_privilegelevel')
#     to_write.pop('credential_cache')
#     to_write.pop('discovered_nodes')
#     obs_file.write(str(to_write))
#     obs_file.write('\n')
#     if not init:
#         obs_file.write("The action is: " + str(gym_action) + '\n')
#         obs_file.write("The reward is: " + str(reward) + '\n')
#         obs_file.write(str(action_type) + '\n')
#     obs_file.write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"+'\n')
...
def write_obs_process(obs_file,observation, gym_action=None, reward=None, action_type=None, init=False):
    ...
    obs_file.write(str(obs_copy['newly_discovered_nodes_count'])+';')
    if obs_copy['leaked_credentials']==():
        obs_file.write(str(0)+';'+str(0)+';'+str(0)+';'+str(0)+';')
    else:
        leaked_cre = obs_copy['leaked_credentials'][0]
        obs_file.write(str(leaked_cre[0])+';'+str(leaked_cre[1])+';'+str(leaked_cre[2])+';'+str(leaked_cre[3])+';')
    obs_file.write(str(obs_copy['lateral_move'])+';')
    obs_file.write(str(obs_copy['credential_cache_length'])+';')
    obs_file.write(str(obs_copy['discovered_node_count']))
    obs_file.write('\n')
