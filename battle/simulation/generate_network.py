...
from simulation.model import Identifiers, NodeID, CredentialID, PortName, FirewallConfiguration, FirewallRule, RulePermission
import numpy as np
...
def new_environment(n_servers_per_protocol: int):
    ...
    network = cyberbattle_model_from_traffic_graph(
        traffic,
        cached_rdp_password_probability=1.0,  #0.8
        cached_smb_password_probability=1.0,  #0.7
        cached_accessed_network_shares_probability=1.0, #0.8
        cached_password_has_changed_probability=0.0,   #0.01
        probability_two_nodes_use_same_password_to_access_given_resource=1.0)  #0.9
