...
def Normalize(v):   #normalize the v
    norm = 0
...
def random_normal_trunc(mean, dev, low, up):   #generate random number and clip it
    x = np.random.normal(mean, dev)
    return np.clip(x, low, up)

class alpha_random_process:                 # the prob of attacker mine the block (consider the deviation)
    ...
    def __init__(self, alpha, dev, interval, name = "iid"):
        ...
        self._dev = dev                            #deviation of alpha
